-- Corrección crítica de seguridad: RLS habilitado en todas las tablas públicas sin políticas
-- ERROR 20: RLS Disabled in Public - Habilitar RLS en tablas restantes

-- Verificar y habilitar RLS en todas las tablas públicas que no lo tengan
ALTER TABLE public.pg_all_foreign_keys ENABLE ROW LEVEL SECURITY;

-- Políticas RLS para tablas de sistema específicas
CREATE POLICY "System can access pg_all_foreign_keys" ON public.pg_all_foreign_keys
  FOR ALL USING (get_current_user_role() = ANY (ARRAY['admin', 'superadmin']));

-- Asegurar que las políticas de tablas críticas están activas
-- (las demás tablas ya deben tener RLS habilitado)

-- Corrección de search_path para funciones restantes de pgtap
