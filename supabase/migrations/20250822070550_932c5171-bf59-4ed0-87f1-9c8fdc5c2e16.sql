...
-- Fix quantum_account_mapping access and restaurant_members infinite recursion

-- 1. Fix quantum_account_mapping - restrict to authenticated users only
-- This is a system mapping table, so only authenticated users should access it
DROP POLICY IF EXISTS "Anyone can view account mapping" ON public.quantum_account_mapping;

-- Keep the existing authenticated users policy, it's more appropriate
-- The existing "Authenticated users can access quantum mappings" policy is sufficient
...
-- 5. Apply audit triggers to sensitive tables (only INSERT, UPDATE, DELETE - not SELECT to avoid performance issues)
DROP TRIGGER IF EXISTS audit_quantum_access ON public.quantum_account_mapping;
CREATE TRIGGER audit_quantum_access
  AFTER INSERT OR UPDATE OR DELETE ON public.quantum_account_mapping
...
CREATE TRIGGER audit_restaurant_valuations_access
  AFTER INSERT OR UPDATE OR DELETE ON public.restaurant_valuations
  FOR EACH ROW EXECUTE FUNCTION public.audit_sensitive_table_access();
...
INSERT INTO public.audit_logs (
  ...
  'SECURITY_FIX',
  'system_security',
  'comprehensive_security_review_fixes_corrected',
  jsonb_build_object(
    'action', 'implemented_critical_security_fixes',
    'timestamp', now(),
    'fixes_applied', jsonb_build_array(
      'quantum_account_mapping_public_access_removed',
      'restaurant_members_infinite_recursion_fixed',
      'enhanced_audit_logging_implemented',
      'sensitive_data_access_triggers_added'
    ),
    'security_level', 'critical',
    'description', 'Comprehensive security review fixes implemented - Phase 1 (Corrected)'
  )
);