-- ===== PIVOTE DEL MODELO DE DATOS: RESTAURANT_ID (CORREGIDO) =====
...
ADD COLUMN IF NOT EXISTS restaurant_id uuid REFERENCES public.franchisee_restaurants(id);
...
ADD COLUMN IF NOT EXISTS franchisee_id uuid REFERENCES public.franchisees(id);
...
WHERE e.id = employee_time_tracking.employee_id
  AND employee_time_tracking.restaurant_id IS NULL;
...
WHERE integration_configs.restaurant_id IS NULL;
...
WHERE fr.id = employees.restaurant_id
  AND employees.franchisee_id IS NULL;
...
WHERE fr.id = annual_budgets.restaurant_id
  AND annual_budgets.franchisee_id IS NULL;
...
WHERE e.id = employee_payroll.employee_id
  AND employee_payroll.franchisee_id IS NULL;
...
WHERE e.id = employee_time_off.employee_id
  AND employee_time_off.franchisee_id IS NULL;
