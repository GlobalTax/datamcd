-- EMERGENCY SECURITY FIXES - Phase 2 (Corrected)
...
-- 4. Restrict integration credentials access - only superadmins
DO $$
BEGIN
  -- Integration configs
  ...
  -- Accounting integration configs
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'accounting_integration_configs') THEN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'delivery_integration_configs') THEN
...
-- 6. Create function to mask sensitive employee data for advisors (fixed position keyword)
...
INSERT INTO public.audit_logs (
  ...
  'system_security',
  'emergency_data_exposure_fixes_corrected',
  jsonb_build_object(
    ...
    'description', 'Emergency fixes for critical data exposure vulnerabilities - corrected'
  )
);